...
def split_classes(batch_size, train_path):
    f = []
    for i in range(4): # Modify the number of class
        tmp = open("class-" + str(i+1) + ".txt", "a") # Modify index i here  
        f.append(tmp)
    
    current_docs = [[], [], [], []] # Modify the number of class
    total_mini_batch = [0, 0, 0, 0] # Modify the number of class
    ...
    with open(train_path) as f_train:
        for i, doc in enumerate(f_train):
            ...
            class_id = int(content[1]) - 1  # Modify index of content and value of subtraction (ie. 1 is starting index of class)
...
def split_holdout(batch_size, datapath=None):
    # print('='*50)
    filenames = ['class-1.txt', 'class-2.txt', 'class-3.txt', 'class-4.txt'] # Modifing here for order of class in training data
...
def concatenate():
    filenames = ['splited_class-1.txt', 'splited_class-2.txt', 'splited_class-3.txt', 'splited_class-4.txt'] # Modifing here for order of class in training data
