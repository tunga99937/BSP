...
def read_data_holdout(f_list_holdout, batch_size):
    list_data = list()

    (wordinds_h_0, wordcnts_h_0, s) = read_minibatch_list_frequencies(f_list_holdout[0], batch_size)
    (wordinds_h_1, wordcnts_h_1, s) = read_minibatch_list_frequencies(f_list_holdout[1], batch_size)
    (wordinds_h_2, wordcnts_h_2, s) = read_minibatch_list_frequencies(f_list_holdout[2], batch_size)
    (wordinds_h_3, wordcnts_h_3, s) = read_minibatch_list_frequencies(f_list_holdout[3], batch_size)
    (wordinds_h_4, wordcnts_h_4, s) = read_minibatch_list_frequencies(f_list_holdout[4], batch_size)
    (wordinds_h_5, wordcnts_h_5, s) = read_minibatch_list_frequencies(f_list_holdout[5], batch_size)

    wordinds_h_0_1, wordcnts_h_0_1, wordinds_h_0_2, wordcnts_h_0_2 = split_data_for_perplex(wordinds_h_0, wordcnts_h_0)
    list0 = (wordinds_h_0_1, wordcnts_h_0_1, wordinds_h_0_2, wordcnts_h_0_2)

    wordinds_h_1_1, wordcnts_h_1_1, wordinds_h_1_2, wordcnts_h_1_2 = split_data_for_perplex(wordinds_h_1, wordcnts_h_1)
    list1 = (wordinds_h_1_1, wordcnts_h_1_1, wordinds_h_1_2, wordcnts_h_1_2)
    
    wordinds_h_2_1, wordcnts_h_2_1, wordinds_h_2_2, wordcnts_h_2_2 = split_data_for_perplex(wordinds_h_2, wordcnts_h_2)
    list2 = (wordinds_h_2_1, wordcnts_h_2_1, wordinds_h_2_2, wordcnts_h_2_2)
    
    wordinds_h_3_1, wordcnts_h_3_1, wordinds_h_3_2, wordcnts_h_3_2 = split_data_for_perplex(wordinds_h_3, wordcnts_h_3)
    list3 = (wordinds_h_3_1, wordcnts_h_3_1, wordinds_h_3_2, wordcnts_h_3_2)
    
    wordinds_h_4_1, wordcnts_h_4_1, wordinds_h_4_2, wordcnts_h_4_2 = split_data_for_perplex(wordinds_h_4, wordcnts_h_4)
    list4 = (wordinds_h_4_1, wordcnts_h_4_1, wordinds_h_4_2, wordcnts_h_4_2)

    wordinds_h_5_1, wordcnts_h_5_1, wordinds_h_5_2, wordcnts_h_5_2 = split_data_for_perplex(wordinds_h_5, wordcnts_h_5)
    list5 = (wordinds_h_5_1, wordcnts_h_5_1, wordinds_h_5_2, wordcnts_h_5_2)

    list_data.append(list3)
    list_data.append(list4)
    list_data.append(list5)
...
def read_setting_forget_drift(file_name):
    f = open(file_name, 'r')
    settings = f.readlines() # a list of lines of setting file, each element is a string - line
    f.close()
    sets = list()
    ...
    ddict['learning_rate'] = float(ddict['learning_rate'])
...
def write_all_holdout_lpp(lpp, file_name):
    write_obj = open(file_name, 'a', newline='')
    csv_writer = writer(write_obj)
    csv_writer.writerow(lpp)
    write_obj.close()
...
def compute_perplexity_holdout(list_holdout, num_holdout, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta):
    holdout_lpp = []
    avg_lpp = 0
    all_lpp = 'all holdout lpp:'
    for i in range(num_holdout):
        (LD, ld) = compute_perplexity( list_holdout[i][0], list_holdout[i][1], list_holdout[i][2], list_holdout[i][3], num_topic, n_term, n_infer, alpha, beta)
        holdout_lpp.append(LD)
        avg_lpp = avg_lpp + LD
        all_lpp = all_lpp + ' ' + str(LD)
    avg_lpp = avg_lpp/num_holdout
    write_perplexities(avg_lpp, avg_holdout_lpp_file)
    write_all_holdout_lpp(holdout_lpp, all_holdout_lpp_file)
    print('avg holdout lpp: ' + str(avg_lpp))
    print(all_lpp)

    return holdout_lpp, avg_lpp 
