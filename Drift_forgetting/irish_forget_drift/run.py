...
def main():
    ...
    # if(len(sys.argv) != 9):
    #     print('Usage: python run.py [mean] [sigma] [rate] [times] [type_mode] [epochs] [iters] [start]')
    #     exit()

    dataset_name = 'concept_drift_irish_holdout'
    times = sys.argv[1]
    ...
    train_file = '/'.join([dataset_dir, 'all-splited-train.txt'])
    setting_file = '/'.join([dataset_dir, 'new_setting.txt'])
    vocab_file = '/'.join([dataset_dir, 'vocab_nonstem.txt'])
    prior_path = '/'.join([dataset_dir, 'prior.glove.200d_nonstem.txt'])
    holdout_list_file = list()
    for i in range(6):
        holdout_file = '/'.join([dataset_dir, 'holdout-' + str(i + 1) + '.txt'])
        holdout_list_file.append(holdout_file)
    # dataset_dir = '/'.join([all_dataset_dir, dataset_name])
    # train_file = '/'.join([dataset_dir, 'train_nonstem.txt'])
    # setting_file = '/'.join([dataset_dir, 'setting'])
    # vocab_file = '/'.join([dataset_dir, 'vocab_nonstem.txt'])
    # prior_path = '/'.join([dataset_dir, 'prior.glove.200d_nonstem.txt'])
    ...
    setting = utilities.read_setting_forget_drift(new_setting_file)
    ...
    class_num = setting['class'].strip().split(' ')
    for i in range(len(class_num)):
        class_num[i] = int(class_num[i])

    f_train = open(train_file, 'r')
    f_list_holdout = list()
    for i in range(6):
        f_holdout = open(holdout_list_file[i], 'r')
        f_list_holdout.append(f_holdout)
    ...
    PPL = []
    holdout_lpp_list = list()
    average_holdout_lpp = list()

    # result file
    all_holdout_lpp_file = '/'.join([GDS_result, 'all_holdout_lpp' + str(mini_batch) + '.csv'])
    avg_holdout_lpp_file = '/'.join([GDS_result, 'avg_holdout_lpp' + str(mini_batch) + '.csv'])
    ...
    # data for test holdout
    list_holdout = utilities.read_data_holdout(f_list_holdout, batch_size) 
    ...
    while True:
        ...
        # Test on holdout    
        if(mini_batch == class_num[3]):
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 1, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
        elif (mini_batch == class_num[3] + class_num[4]):
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 2, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
        elif (mini_batch == class_num[3] + class_num[4] + class_num[5]):
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 3, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
        elif (mini_batch == class_num[3] + class_num[4] + class_num[5] + class_num[0]):
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 4, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
        elif (mini_batch == class_num[3] + class_num[4] + class_num[5] + class_num[0] + class_num[1]):
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 5, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
        elif stop == 1:
            holdout_lpp, avg_lpp = utilities.compute_perplexity_holdout(list_holdout, 6, avg_holdout_lpp_file, all_holdout_lpp_file, num_topic, n_term, n_infer, alpha, beta_minibatch)
            holdout_lpp_list.append(holdout_lpp)
            average_holdout_lpp.append(avg_lpp)
            print('all lpp :')
            print(holdout_lpp_list)
            print('avg lpp :')
            print(average_holdout_lpp)
            break
        ...
        print('Minibatch ' + str(mini_batch) + ' |=====| PERPLEXITY ' + dataset_name + ' : ' + str(LD) +  '\n')
        PPL.append(LD)
