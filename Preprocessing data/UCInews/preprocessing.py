...
if __name__ ==  '__main__':
	...
	num_doc =0
	print(len(we))
	fpt = open('%s\\uci-news-raw.txt'%(folder), 'w')
	print('Preprocessing')
	with open(filein) as csv_file:
		...
		for row in csv_reader:
			num_doc+=1
			timestamp = datetime.datetime.fromtimestamp(int(row[7])/1000)
			fpt.write(timestamp.strftime("%Y%m%d")+' ')
			strl = row[4]
			if strl in label:
	...
	print(label)
	print(num_doc)
	with open('%s\\vocab.txt'%(folder), 'w') as fp:
		for w in vocab:
			if vocab[w]>2:
				fp.write(w+'\n')
