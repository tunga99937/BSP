...
if __name__ ==  '__main__':
	...
	vocab = {}
	index = 0
	fpp = open('%s/prior.glove.200d.txt'%(folder), 'w')
	with open(filevocab, 'r') as fp:
		while True:
			...
			index+=1
	fpp.close()
	fpt = open('%s/train.txt'%(folder), 'w')
	fpf = open('%s/trainconcept.txt'%(folder), 'w')
	n = 0
	with open(filein, 'r') as fp:
		while True:
			...
			if len(doc)>1:
				fpt.write('%s '%len(doc))
				fpf.write('%s %s '%(n,label))
				for w in doc:
					fpt.write('%s:%s '%(vocab[w], doc[w]))
					fpf.write('%s:%s '%(vocab[w], doc[w]))
				fpt.write('\n')
				fpf.write('\n')
	fpt.close()
	fpf.close()
